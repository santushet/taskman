...
type RestOf<T extends ITask | NewTask> = Omit<T, 'startDateTime' | 'endDateTime' | 'dueDate'> & {
...
export class TaskService {
  ...
  protected convertDateFromClient<T extends ITask | NewTask | PartialUpdateTask>(task: T): RestOf<T> {
    return {
      ...task,
      startDateTime: task.startDateTime?.toJSON() ?? null,
      endDateTime: task.endDateTime?.toJSON() ?? null,
  ...
  protected convertDateFromServer(restTask: RestTask): ITask {
    return {
      ...restTask,
      startDateTime: restTask.startDateTime ? dayjs(restTask.startDateTime) : undefined,
      endDateTime: restTask.endDateTime ? dayjs(restTask.endDateTime) : undefined,
